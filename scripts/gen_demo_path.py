...
def kt(*pairs):
    ...
    ks = ";".join(
        f"{k:.4f}".rstrip("0").rstrip(".") if k not in (0, 1) else str(int(k)) for k, _ in pairs
    )
...
e(
    f'<rect x="1" y="1" width="{W - 2}" height="{H - 2}" rx="14" fill="url(#bg)" stroke="{BORDER}" stroke-width="1.5"/>'
)
...
e(
    f'<text x="{PANEL - 16}" y="34" text-anchor="end" font-size="10" fill="{TXT_DIM}">graphify</text>'
)
...
for i, ch in enumerate(cmd):
    ...
    e(
        f'<text x="{cx:.1f}" y="{line_y}" font-size="{FS}" fill="{TXT}" opacity="{op0()}">{disp}{reveal(t)}</text>'
    )
...
if not STATIC:
    e(
        f'<rect y="{line_y - 9}" width="7" height="12" fill="{GREEN}" opacity="0.9">'
...
for i, (txt, col, t) in enumerate(out_lines):
    ...
    e(
        f'<text x="{LX}" y="{y}" font-size="{FS}" fill="{col}" opacity="{op0()}" '
        f'style="white-space:pre">{disp}{reveal(t)}</text>'
    )

cap_y = H - 30
e(
    f'<text x="{LX}" y="{cap_y}" font-size="12" fill="{GREEN_HI}" font-weight="600" opacity="{op0()}">'
...
while len(distractors) < 26 and attempts < 4000:
    ...
    if all((x - ax) ** 2 + (y - ay) ** 2 > 44**2 for ax, ay in avoid) and all(
        (x - dx) ** 2 + (y - dy) ** 2 > 34**2 for dx, dy in distractors
    ):
...
for i, (x, y) in enumerate(distractors):
    dists = sorted(
        range(len(allpts)), key=lambda j: (allpts[j][0] - x) ** 2 + (allpts[j][1] - y) ** 2
    )
    for j in dists[1:3]:
...
for i, j in dim_edges:
    ...
    x2, y2 = allpts[j]
    e(
        f'<line x1="{x1:.1f}" y1="{y1:.1f}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="{DIM_EDGE}" stroke-width="1"/>'
    )
...
for i in range(3):
    ...
    q = path_nodes[i + 1][1]
    e(
        f'<line x1="{p[0]}" y1="{p[1]}" x2="{q[0]}" y2="{q[1]}" stroke="{DIM_EDGE}" stroke-width="1.2"/>'
    )
...
for i in range(3):
    ...
    anim = (
        ""
        if STATIC
        else f'<animate attributeName="stroke-dashoffset" values="{vs}" keyTimes="{ks}" dur="{T}s" repeatCount="indefinite"/>'
    )
    e(
        f'<line x1="{p[0]}" y1="{p[1]}" x2="{q[0]}" y2="{q[1]}" stroke="{GREEN}" stroke-width="2" '
        f'stroke-linecap="round" stroke-dasharray="{L:.1f}" stroke-dashoffset="{off0}">{anim}</line>'
    )
    mx, my = (p[0] + q[0]) / 2, (p[1] + q[1]) / 2 - 6
    e(
        f'<text x="{mx:.1f}" y="{my:.1f}" font-size="9" fill="{GREEN_HI}" text-anchor="middle" opacity="{op0()}" '
...
for (name, (x, y), pos), t in zip(path_nodes, path_light_t):
    ...
    ganim = (
        ""
        if STATIC
        else f'<animate attributeName="opacity" values="{gvs}" keyTimes="{gks}" dur="{T}s" repeatCount="indefinite"/>'
    )
    ...
    e(
        f'<text x="{x}" y="{ly}" font-size="10.5" fill="{TXT}" text-anchor="middle" opacity="{op0()}" '
