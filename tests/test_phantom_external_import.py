...
def test_scoped_package_import_is_ref_namespaced():
    tgt, resolved_path = _resolve_js_import_target("@scope/utils", "src/thing.ts")
    assert resolved_path is None
...
def test_no_phantom_edge_from_tsx_to_unrelated_python_file(tmp_path: Path):
    ...
    py_ids = [
        n for n, d in G.nodes(data=True) if str(d.get("source_file", "")).endswith("colors.py")
    ]
    ...
    for u, v, d in G.edges(data=True):
        ...
        if endpoints & set(py_ids):
            ...
            assert not any(sf.endswith((".tsx", ".ts")) for sf in srcfiles), (
                f"phantom cross-language imports_from edge onto colors.py: {u} -> {v} ({d})"
            )
...
def test_multiple_tsx_files_do_not_all_alias_onto_one_python_file(tmp_path: Path):
    ...
    for i in range(3):
        _write(
            ...
            f'import colors from "tailwindcss/colors";\nexport const C{i} = colors.blue;\n',
        )
