    ...
    nodes = [
        {
            ...
            "label": "index.ts",
            "file_type": "code",
            "source_file": "fn/process-order/index.ts",
        },
        {
            "id": "sr",
            "label": "index.ts",
            "file_type": "code",
            "source_file": "fn/send-receipt/index.ts",
        },
...
def test_is_file_node_label_and_suffix_helpers():
    ...
    assert not _is_file_node_label("index", "a/b/index.ts")
    ...
    assert (
        _shortest_unique_suffix("supabase/functions/process-order/index.ts", all_sfs)
...
def test_colliding_file_labels_are_qualified_uniques_left_bare():
    ...
    G.add_node(
        ...
        label="handler",
        file_type="code",
        source_file="supabase/functions/process-order/index.ts",
    )
...
def test_end_to_end_build_and_lookup(tmp_path):
    ...
    (fns / "process-order" / "index.ts").write_text(
    ...
    result = extract(
        [
            fns / "process-order" / "index.ts",
            fns / "send-receipt" / "index.ts",
            tmp_path / "main.ts",
        ],
