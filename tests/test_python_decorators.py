...
def test_attribute_decorator_targets_the_symbol_not_the_module(tmp_path):
    f = _write(
        tmp_path / "pkg" / "web.py", 'import app\n\n@app.route("/")\ndef index():\n    pass\n'
    )
    r = extract([f], cache_root=tmp_path)
...
def test_decorated_method_owner_is_class_qualified(tmp_path):
    ...
    r = extract([f], cache_root=tmp_path)
    assert _make_id("traced") in _deco_edges(r, _method_nid("pkg/svc.py", "Service", "handle"))


def test_property_still_class_qualified(tmp_path):
    f = _write(
        tmp_path / "pkg" / "prop.py",
        "class Config:\n    @property\n    def name(self):\n        return 1\n",
    )
    r = extract([f], cache_root=tmp_path)
    assert any(n["id"] == _method_nid("pkg/prop.py", "Config", "name") for n in r["nodes"])
...
def test_decorated_class(tmp_path):
    ...
    r = extract([f], cache_root=tmp_path)
    assert _make_id("register_model") in _deco_edges(r, _class_nid("pkg/model.py", "Point"))
...
def test_functools_wraps_does_not_rewire_onto_local_wraps(tmp_path):
    _write(tmp_path / "pkg" / "gift.py", "def wraps(thing):\n    return thing\n")
    f = _write(
