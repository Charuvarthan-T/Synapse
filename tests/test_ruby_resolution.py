...
def test_constant_receiver_singleton_call_resolves(tmp_path: Path) -> None:
    ...
    runner = _write(
        tmp_path, "runner.rb", "class Runner\n  def run\n    Processor.call\n  end\nend\n"
    )
...
def test_ambiguous_constant_receiver_emits_no_edge(tmp_path: Path) -> None:
    ...
    caller = _write(tmp_path, "c.rb", "class Runner\n  def run\n    Processor.call\n  end\nend\n")
    graph = extract(
        [caller, tmp_path / "a.rb", tmp_path / "b.rb"], cache_root=tmp_path, parallel=False
    )
...
def test_include_emits_mixes_in_edge(tmp_path: Path) -> None:
    ...
    _write(
        tmp_path, "model.rb", "class Roster < ApplicationRecord\n  include SealedProtection\nend\n"
    )
    g = extract(
        [tmp_path / "model.rb", tmp_path / "concern.rb"], cache_root=tmp_path, parallel=False
    )
...
def test_rake_files_extract_and_resolve_like_rb(tmp_path):
    ...
    rake = _write(
        tmp_path, "ops.rake", "class RakeHelper\n  def self.run\n    Widget.tally\n  end\nend\n"
    )
