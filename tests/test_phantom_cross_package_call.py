...
def test_many_files_do_not_collapse_onto_one_export(tmp_path: Path) -> None:
    _write(tmp_path / "proto/index.ts", "export function encode(x: string): string { return x; }\n")
    for i in range(4):
        _write(
            tmp_path / f"svc{i}/index.ts",
