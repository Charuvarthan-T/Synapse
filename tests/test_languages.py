...
def test_cuda_finds_kernel_and_device_functions():
    ...
    assert any("dot" in l for l in labels)
...
def test_nested_types_contained_by_enclosing_type(tmp_path):
    ...
    assert ("Outer.java", "Outer") in cj
    assert ("Outer", "Inner") in cj
    assert ("Outer.java", "Inner") not in cj
...
def test_swift_extension_across_files_merges_into_canonical_type():
    ...
    assert any("one" in l for l in method_labels), (
        f"one() should attach to Foo, got {method_labels}"
    )
    assert any("two" in l for l in method_labels), (
...
def test_fortran_case_insensitive_names():
    ...
    labels = [n["label"] for n in r["nodes"]]
    assert all(
        l == l.lower() or "(" in l for l in labels if l.endswith(("()", "")) and not "." in l
    )
    assert "geometry" in labels
    assert "main" in labels
...
def test_markdown_fenced_heading_not_parsed():
    ...
    assert any("Another Real Heading" in l for l in labels), (
...
def test_swift_extension_folds_onto_objc_class():
    ...
    assert any("describe" in l for l in labels), (
