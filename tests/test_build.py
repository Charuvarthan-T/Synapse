...
def test_null_weight_edge_builds_and_clusters(tmp_path):
    ...
    assert G["a"]["b"]["confidence_score"] == 1.0
...
def test_malformed_weights_normalize():
    ...
    extraction = {
        "nodes": [
            {"id": f"n{i}", "label": str(i), "file_type": "code", "source_file": f"{i}.py"}
            for i in range(4)
        ],
        "edges": [
    ...
    G = build_from_json(extraction)
    assert G["n0"]["n1"]["weight"] == 3.5
    assert G["n1"]["n2"]["weight"] == 1.0
    assert G["n2"]["n3"]["weight"] == 1.0
...
def test_legacy_node_name_path_aliases_folded():
    ...
    ext = {
        "nodes": [{"id": "n1", "name": "Foo", "path": "a/b.md", "file_type": "concept"}],
        "edges": [],
        "input_tokens": 0,
        "output_tokens": 0,
    }
...
def test_ghost_merge_non_ast_different_files_both_survive():
    ...
    ext = {
        "nodes": [
            {
                "id": "dir_a_update_build_merge",
                "label": "build_merge() function",
                "file_type": "concept",
                "source_file": "dir_a/update.md",
                "source_location": "L10",
            },
            {
                "id": "dir_b_update_build_merge",
                "label": "build_merge() function",
                "file_type": "concept",
                "source_file": "dir_b/update.md",
                "source_location": "L12",
            },
        ],
        "edges": [],
...
def test_build_from_json_ambiguous_old_stem_alias_stays_dangling(tmp_path):
    ...
    extraction = {
        "nodes": [
            {
                "id": "dev_monitoring_ping",
                "label": "ping.h",
                "file_type": "code",
                "source_file": "Dev/monitoring/ping.h",
            },
            {
                "id": "www_pages_api_ping",
                "label": "ping.php",
                "file_type": "code",
...
def test_build_from_json_ambiguous_alias_detected_despite_header_impl_salting(tmp_path):
    ...
    extraction = {
        "nodes": [
            {
                "id": "tools_aolserver_utility_h_tools_aolserver_utility",
                "label": "utility.h",
                "file_type": "code",
                "source_file": "Tools/aolserver/utility.h",
            },
            {
                "id": "tools_aolserver_utility_cpp_tools_aolserver_utility",
                "label": "utility.cpp",
                "file_type": "code",
                "source_file": "Tools/aolserver/utility.cpp",
            },
            {
                "id": "wwwapi_masque_com_pages_utility",
                "label": "utility.php",
                "file_type": "code",
...
def test_cross_language_imports_references_are_dropped():
    ...
    G = build_from_json(ext, directed=False)
    assert not G.has_edge("backend_worker_py", "src_time_ts"), (
        "cross-language import must be dropped"
    )
...
def test_markdown_doc_twin_merges_into_semantic_doc_node():
    ...
    G = build_from_json(ext, directed=False)
    assert "docs_readme" not in G.nodes()
    assert "docs_readme_doc" in G.nodes()
    assert G.has_edge("docs_guide", "docs_readme_doc")
    assert G.has_edge("docs_readme_doc", "code_auth")
