...
def test_file_within_size_cap(tmp_path):
    ...
    assert detect._file_within_size_cap(f) is True
    assert detect._file_within_size_cap(f, cap=512) is False
...
def test_zip_ratio_bomb_rejected(tmp_path):
    ...
    _write_zip(bomb, "xl/worksheets/sheet1.xml", b"0" * (5 * 1024 * 1024))
    assert bomb.stat().st_size < 100 * 1024
...
def test_streaming_ceiling_rejects_oversized_actual(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setattr(detect, "_OFFICE_MAX_DECOMPRESSED_BYTES", 64 * 1024)
    f = tmp_path / "big.xlsx"
    import os as _os
