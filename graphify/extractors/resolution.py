...
def _resolve_tsconfig_alias(
    raw: str, aliases: dict[str, list[str]], base_url: "Path | None" = None
) -> "Path | None":
...
def _resolve_js_module_path(raw: str | Path, start_dir: Path | None = None) -> Path | None:
    ...
    hit = _resolve_tsconfig_alias(raw, aliases, base_url=_load_tsconfig_base_url(start_dir))
    if hit is not None:
...
def _resolve_php_type_references(
    ...
    for edge in all_edges:
        ...
        still_referenced.add(edge.get("target"))
    all_nodes[:] = [
        n for n in all_nodes if n.get("id") not in repointed_from or n.get("id") in still_referenced
    ]
