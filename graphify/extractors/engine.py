            ...
            if c.type == "simple_user_type":
                for sub in c.children:
                    if sub.type in ("identifier", "type_identifier"):
                        ...
                        if (
                            text
                            and text not in _KOTLIN_BUILTIN_TYPES
                            and text not in _JAVA_BUILTIN_TYPES
                        ):
                            out.append((text, "generic_arg" if generic else "type"))
                        break
...
def _ts_receiver_type_table(root, source: bytes, table: dict[str, str]) -> None:
    ...
    def _bare_type_ident(type_annotation):
        ...
        others = [
            c for c in type_annotation.children if c.is_named and c.type not in ("type_identifier",)
        ]
            ...
            if config.ts_module == "tree_sitter_ruby":
                ...
                if _rb_body is not None:
                    for _stmt in _rb_body.children:
                        if (
                            _stmt.type != "call"
                            or _stmt.child_by_field_name("receiver") is not None
                        ):
                            continue
                        _m = _stmt.child_by_field_name("method")
                        if _m is None or _read_text(_m, source) not in (
                            "include",
                            "extend",
                            "prepend",
                        ):
            ...
            if config.ts_module == "tree_sitter_python":
                ...
                if inner_name and normalize_id(inner_name):
                    ...
                    for child in node.children:
                        ...
                        if target != owner_nid:
                            add_edge(
                                owner_nid, target, "references", deco_line, context="decorator"
                            )
            for child in node.children:
